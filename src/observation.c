...
ListeObservation *AddObservation(Patient *patientEnConsultation, ListeObservation *ListeO, ListeLit *ListeL) {
    ...
    int duree;
    printf("Duree de l'observation (en jours) : ");
    scanf("%d", &duree);
    patientOB->debutObservation = time(NULL);
    patientOB->finObservation   = patientOB->debutObservation + (duree * 24 * 60 * 60); // convertir jours en secondes
    printf("Indiquez le traitement a suivre : ");
    scanf(" %[^\n]", patientOB->traitement);
    patientOB->debutObservation = time(NULL);
    patientOB->lit = AddLit(patientOB,ListeL);
    patientOB->patient->etat = OBSERVATION;
