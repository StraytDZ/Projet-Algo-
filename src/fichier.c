...
void sauvegarderPatients(Patient *patient)
{
    ...
    if (patient->etat == CONSULTATION)
        data.debutConsultation = patient->debutConsulation;
...
void chargerPatients(ListePatient *liste, ListeTicket *listeT)
{
    ...
    while (fread(&data, sizeof(PatientData), 1, f) == 1)
    {
        ...
        if (p->etat == ATTENTE)
            liste->attente++;
        else if (p->etat == OBSERVATION)
            liste->observation++;
        else if (p->etat == SORTI)
            liste->sortis++;
        else if (p->etat == TRANSFERER)
            liste->transferes++;
    }
...
void afficherHistorique()
{
    ...
    while (fread(&data, sizeof(Data), 1, f) == 1)
    {
        ...
        if (data.etat == SORTI)
            etatStr = "SORTI";
        else if (data.etat == TRANSFERER)
            etatStr = "TRANSFERE";
        else if (data.etat == CONSULTATION)
            etatStr = "CONSULTATION";
        else if (data.etat == ATTENTE)
            etatStr = "ATTENTE";
        else if (data.etat == OBSERVATION)
            etatStr = "OBSERVATION";
        else if (data.etat == URGENCE)
            etatStr = "URGENCE";
        else
            etatStr = "INCONNU";
